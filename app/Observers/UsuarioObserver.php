...
class UsuarioObserver
{
    ...
    public function updating(Usuario $usuario)
    {
        ...
        Log::channel('acessos')->info("Coluna 'status' do usuario {$usuario->id} foi modificada de {$usuario->getOriginal('status')} para {$usuario->status} pelo usuario " . auth()->user()->id);
      }
    }
