...
@php
  ...
  $descontos = \DB::select("select ifnull(sum(valor),0) as descontos from descontos where id_rep in ($representante) and ano = $ano and periodo = $periodo and tipo  in ('Adiantamento','Leitor', 'Antecipação','Diferença pagamento') ");

	$ajuste_nf = \DB::select("select ifnull(sum(valor),0) as ajustenf from descontos where id_rep in ($representante) and ano = $ano and periodo = $periodo and tipo  in ('Ajuste nf') ");
	$ajuste_nf1 = $ajuste_nf[0]->ajustenf;
$diferenca_nf = \DB::select("select ifnull(sum(valor),0) as diferenca_nf from descontos where id_rep in ($representante) and ano = $ano and periodo = $periodo and tipo  in ('Diferença pagamento') ");
	$diferenca_nf = $diferenca_nf[0]->diferenca_nf;
  ...
  <div class="col-md-8">
    ...
    <div class="box box-body box-widget">
      <div class="row">
		...
		<div class="col-md-4">
          <label>Código representante: </label>{{ $representante}}
        </div>
    ...
    <div class="box box-widget">
      ...
      <div class="box-body">
        <table class="table table-bordered">
            ...
            <td width="20%" class="text-bold" align="right">{{number_format($total_comissao-$diferenca_nf,2,',','.')}}</td>
          </tr>
          <tr>
            <td width="80%" class="text-bold">Valor da Comissão</td>
            <td width="20%" class="text-bold" align="right">{{number_format($total_comissao+$ajuste_nf1,2,',','.')}}</td>
          </tr>
          <tr>
...
@php
  ...
  $descontos = \DB::select("select * from descontos where id_rep in ($representante) and ano = $ano and periodo = $periodo and tipo  in ('Adiantamento','Leitor', 'Antecipação','Diferença pagamento','Ajuste NF') and valor > 0");
@endphp
