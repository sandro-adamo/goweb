...
@section('title')
  <i class="fa fa-gears"></i> Novo Inventário
...
@if (isset($_GET["duplicado"]))
	...
	<div class="callout callout-warning">
		...
		<a href="/mostruarios/inventarios/novo" class="btn btn-danger"><i class="fa fa-close"></i> Não</a>
	</div>
		...
		<div class="col-md-8">
			...
			<div class="box box-widget">
				...
				<div class="box-header with-border">
					...
					<button type="button" class="btn btn-xs btn-default pull-right btn-flat " id="btnImporta"><i class="fa fa-upload"></i> Importar Inventário</button>
					
					 
					<button type="button" class="btn btn-xs btn-default pull-right btn-flat " data-toggle="modal" data-target="#modalImportaDevolucao"><i class="fa fa-upload"></i> Importar Devolução</button>
					 
					@endif

					<span class="pull-right"><a href="/mostruarios/inventarios/{{$id_inventario}}/exportainventario" class="btn btn-xs btn-default"><i class="fa fa-file-o"></i> Exportar Inventário1</a>
						...
						@foreach ($itens as $peca)
							...
							<tr>
								...
								<td width="25%" align="center">@if ($peca->acao == 'DEVOLVER')
								 ...
								 <span class="text-red text-bold" data-value="{{$peca->id}}">{{$peca->acao}}</span> 
								
								 @else
								 @if (isset($itens[0]->status) && $itens[0]->status=="Iniciada")

								  
								  <a href="" class="text-green text-bold alteraSituacao" data-value="{{$peca->id}}">{{$peca->acao}}</a>
								  @else
								  <span class="text-green text-bold" >{{$peca->acao}}</span>
								...
								<td width="25%" align="center">
									<a href="/mostruarios/inventarios/{{$peca->id}}/excluir" class="text-red"> <i class="fa fa-trash"></i> Excluir </a>									
								</td>
							</tr>
...
{{--       	<div class="callout callout-warning">
      		<span class="lead"> Após o envio da lista de devolução, não será possível o envio e retirada de itens do inventário!</span>
      	</div> --}}
...
<form action="/mostruarios/inventarios/importa" id="frmImporta" method="post" enctype="multipart/form-data">
    @csrf 
<div class="modal fade" id="modalImportaItens" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Importar invetário </h4>
      </div>
      <div class="modal-body">
...
<form action="/mostruarios/inventarios/importaDevolucao" id="frmImporta" method="post" enctype="multipart/form-data">
    @csrf 
...
<form action="/mostruarios/inventarios/altera" id="frmImporta" method="post" class="form-horizontal">
    @csrf 
<div class="modal fade" id="modalAlteraSituacao" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
