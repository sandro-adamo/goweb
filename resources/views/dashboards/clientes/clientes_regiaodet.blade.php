...
$query_1 = \DB::select("						
   ...
   select municipio, uf, populacao,  faixa_cep, cod_cliente, cliente, count(codcli) pdvs, sum(v120) v120, (sum(v120)/max(populacao))*1000 pc1000 from (
		select ab.municipio, ab.uf, populacao,  left(ab.cep,2) faixa_cep, cart.cod_cliente, ab.cliente, cart.codcli, sum(v120) v120
		...
		where cart.rep_carteira in ($representantes) and ab.municipio = '$municipio'
        group by ab.municipio, ab.uf, left(ab.cep,2) ,  cart.cod_cliente, ab.cliente, populacao, cart.codcli
	) as fim
	where faixa_cep = '$faixa_cep' 
    group by municipio, uf, populacao, faixa_cep, cod_cliente, cliente
...
<div class="row">
		...
		<div class="col-md-7">
			<div class="box box-widget box-body">
				...
				<td>{{$query1->uf}}</td>
				<td align="left">{{$query1->municipio}}</td>
				
				<td align="center"><a href="/det_subgrupo?pdv={{$query1->cod_cliente}}">{{$query1->cliente}}</a></td>
