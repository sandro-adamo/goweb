...
select * from (	
	...
	0 prev_icms,	0 prev_total,	null coloremb, null 	clasmoney, null 	embarque,
		...
		(select (pedido) from importacoes_pedidos imp where imp.ref_go = ci.num_temp limit 1) vinculo, '' acao2
...
select * from (
		select base1.*, 
			case when id_pedido_temp is not null and id_pedido_jde is not null then 'ERRO'
			when id_pedido_jde is not null then 'update' when id_pedido_temp is not null then 'vincular'
			else 'insert' end as acao_capa,
			 
			case when id_jde is not null then id_jde else id_temp end as vinculo_infos

		from (
			select distinct pedido, tipo, ref_go ,
			infosjde.id id_jde, infosjde.id_pedido id_pedido_jde, infosjde.tipo_pedido tipo_pedido_jde, infosjde.num_temp  num_temp_jde,
			infostemp.id id_temp, infostemp.id_pedido id_pedido_temp, infostemp.tipo_pedido tipo_pedido_temp, infostemp.num_temp num_temp_temp 
...
<h6> 
	...
	<thead>				
		...
		@foreach ($query_teste as $queryt)
				...
				@php if($queryt->acao_capa=='ERRO') 
					{$color='red';
					$icone='fa fa-exclamation-circle';} 

				elseif($queryt->acao_capa=='update') 
					{$color='green';
					$icone='fa fa-list-alt';}

				elseif($queryt->acao_capa=='insert') 
					{$color='blue';
					$icone='fa fa-file-o';}
		...
		<tr>
			...
			<input type="hidden" id="id_temp" name="id_temp" size="50" value={{$queryt->id_temp}} >
		...
		<td align="left">
			<a href="/import_form/?tipo={{$queryt->tipo}}&pedido={{$queryt->pedido}}" target="_blank">{{$queryt->tipo.' '.$queryt->pedido}}</a>
		</td>

		<td><button type="submit"><i class="{{$icone}} text-{{$color}}"></i></button>	</td> 


		<td align="left">{{$queryt->id_jde}}</td>
		<td align="left">{{$queryt->id_temp}}</td>
		<td align="left">{{$queryt->id_pedido_temp}}</td>
			...
			@endforeach 
					...
					<table class="tabela2 table-striped table-bordered compact" id="myTable7">
						<thead>	
							...
							<td align="left">{{$query3->fornecedor}}</td>
							<td align="center">{{$query3->tipoitem}}</td>
							<td align="center">{{$query3->acao2}}</td>
							<td align="center">{{$query3->colmod}}</td>
							<td align="center">{{$query3->linha}}</td>
							<td align="center">{{number_format($query3->qtde)}}</td>	
							<td align="center">{{number_format($query3->atende)}}</td>
