...
select id_cliente cod_cliente, fantasia, uf, municipio, dt_emissao, secundario, sum(qtde) qtde 
    ...
    where datediff(now(),dt_emissao) > 5 and secundario = '$item' and uf = '$uf'
...
$query_2 = \DB::select("
	...
	select municipio,  sum(qtde) qtde 
	from notas_jde nfs 
	left join itens on itens.id = nfs.id_item 
    left join addressbook ab on ab.id = nfs.id_cliente
    where datediff(now(),dt_emissao) > 5 and secundario = '$item' and uf = '$uf'
...
<div class="row">
	...
	<div class="col-md-4">
		<div class="box box-body box-widget">
			<table class="table table-bordered"> 
		 <tr>	
	 		<td colspan="8">Clientes que compraram o item {{$item}} por municipio</td>
				</tr>
		  <tr>	
			...
			@foreach ($query_2 as $query2)
				<tr>
					<td align="center"><a href="/itens_clientes_municipio?municipio={{$query2->municipio}}&item={{$item}}&uf={{$uf}}">{{$query2->municipio}}</a></td>
