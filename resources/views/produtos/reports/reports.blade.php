...
@php
$sql = DB::select("
Select left(agrup,4) agrup, 
format(disponivel/total*100,0) disponivel, 
format(15_dias/total*100,0) dias15, 
format(30_dias/total*100,0) dias30, 
format(producao/total*100,0) Producao, 
format(esgotado/total*100,0) Esgotado
from(


select Agrup, format(sum(Producao),2) Producao, format(sum(15_dias),2) 15_dias, format(sum(30_dias),2) 30_dias, format(sum(Disponivel),2) Disponivel, format(sum(esgotado),2) Esgotado,
format(sum(Producao),2)+format(sum(15_dias),2)+format(sum(30_dias),2)+format(sum(Disponivel),2)+format(sum(esgotado),2) as Total
from(
select Agrup, statusatual,
case when statusatual = 'AGUARDAR PRODUÇÃO' then count(secundario) else 0 end as 'Producao',
case when statusatual = 'disponivel em 15 DIAS' then count(secundario) else 0 end as '15_dias',
case when statusatual = 'disponivel em 30 DIAS' then count(secundario) else 0 end as '30_dias',
case when statusatual = 'entrega imediata' then count(secundario) else 0 end as 'Disponivel',
...
function drawStacked() {
      var data = google.visualization.arrayToDataTable([
        ['Agrup', 'Disponivel',{ role: 'style' },'15 dias',{ role: 'style' },'30 dias',{ role: 'style' },'Produção',{ role: 'style' },'Esgotado',{ role: 'style' }],
         @foreach ($sql as $dados) 

            @php 	
            $agrupamento = $dados->agrup;
            $disponivel = $dados->disponivel;
			$dias15 = $dados->dias15;
			$dias30 = $dados->dias30;
			$esgotado = $dados->Esgotado;
		   ...
		   ['{{$agrupamento}}',{{$disponivel}} , '#A5DF00', {{$dias15}} , '#F2F5A9', {{$dias30}} , '#FFFF00', {{$producao}}, '#DF7401', {{$esgotado}}, '#DF0101'],
	...
	@endforeach
      ...
      var options = {
        ...
        isStacked: true,
		  ...
		  colors: ['#A5DF00','#F2F5A9', '#FFFF00', '#DF7401', '#DF0101'],
        vAxis: {
          title: 'Agrupamentos'
        }
      };
...
//			@endphp
	...
	@endforeach
      ...
      chart.draw( data, options);
		...
		<div id="chart_div2" style="width: 900px; height: 900px;"></div>         
    ...
    <script type="text/javascript">
     ...
     google.charts.load('current', {'packages':['table']});
      ...
      function drawTable() {
		...
		['{{$agrupamento}}',  {v: {{$Atende_disp}}},  {v: {{$Atende_TO}}},  {v: {{$Atende_CET}}},  {v: {{$Atende_ETQ}}},  {v: {{$Atende_CEP}}},  {v: {{$Total_Atende}}},  {v: {{$Orc_Pendente}}}],
